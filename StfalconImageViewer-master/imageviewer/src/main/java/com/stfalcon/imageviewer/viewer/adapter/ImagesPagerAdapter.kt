...
class ImagesPagerAdapter<T>(
    ...
    private val imageItemRatio: GetViewRatio,
    ...
    fun isScaled(position: Int): Boolean = holders.firstOrNull {
        it.adapterPosition == position
    }?.isScaled ?: false

    fun scaledSize(position: Int): Float = holders.firstOrNull {
        it.adapterPosition == position
    }?.scaledSize ?: 1.0f

    fun isTopOrBottom(position: Int): Int = holders.firstOrNull {
        it.adapterPosition == position
    }?.topOrBottom ?: IMAGE_POSITION_DEFAULT

    fun isInitState(position: Int) = holders.firstOrNull {
        it.adapterPosition == position
    }?.isInitState ?: true

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val itemView = onCreateView.createView(context, viewType)
        return ViewHolder(itemView, viewType).also {
            holders.add(it)
        }
    }
    ...
    fun getItemViewRatio(position: Int): Float {
        return imageItemRatio.getItemViewRatio(position)
    }
            ...
            when (viewType) {
                ...
                VIEW_TYPE_SUBSAMPLING_IMAGE -> {
                    ...
                    val subsamplingScaleImageView =
                        viewGroup.getChildAt(0) as SubsamplingScaleImageView
                    subsamplingScaleImageView.setOnStateChangedListener(object :
                        ...
                        override fun onCenterChanged(newCenter: PointF?, origin: Int) {
                            subsamplingScaleImageView.sWidth   // 源文件宽
